...
\frametitle{\ }
\vspace{-7.5mm}
\begin{Definition}[Probability and Related Terms (\only<1>{1}\only<2>{2}/2)]
    \begin{itemize}
    \only<1>{
...
\footnotesize
  \begin{exampleblock}{Classic Example: Medical Test}
    ...
    Therefore, $P[\text{Disease}|\text{Positive}] = \frac{0.99 \times 0.01}{0.0594} \approx 0.1667$ or \textbf{only 16.7\%!}
...
\begin{alertblock}{Bridge to the Next Topic}
    With the foundational theory and calculation tools in hand, we are ready to move to the next stage: modeling the sequence of tokens in a comment as a \textbf{sequential experiment} and numerical features using random variables.
\end{alertblock}
\end{frame}
